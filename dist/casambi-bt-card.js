...
function scoreEntity(s, wanted, network, type, config = {}) {
  ...
  if (wantedSlug && eid.includes(wantedSlug)) { score += 35; reasons.push('entity contains unit slug'); }
  ...
  if (source !== 'casambi_bt_candidate') { score -= 100; reasons.push('external source penalty'); }
  if (type === 'scene' && networkSlug && wantedSlug && eid === `scene.${networkSlug}_${wantedSlug}`) { score += 130; reasons.push('casambi scene pattern'); }
...
function rawTargets(hass, config, type) {
  ...
  if (type === 'light') return { targets: (raw.network.units || []).map((u) => ({ name: u.name, id: u.deviceID || u.deviceId, uuid: u.uuid })).filter((x) => x.name), network };
  return { targets: (raw.network.scenes || []).map((s) => ({ name: s.name, id: s.sceneID || s.sceneId })).filter((x) => x.name), network };
}
...
function candidatesFor(hass, config, type) {
  ...
  if (config.discovery?.include_fallback_candidates !== false) {
    for (const state of all) {
...
function selectedBest(hass, config, type, onlyAvailable = false) {
  ...
  const manual = normList(config[key]).filter((e) => hass.states[e]);
...
const PRESET_OPTIONS = ['casambi-native', 'casambi-neon', 'mushroom-soft', 'mushroom-dark', 'bubble-compact', 'bubble-glass', 'minimal'];
...
class CasambiBtCard extends HTMLElement {
  ...
  static getStubConfig() { return { title: 'Casambi', preset: 'casambi-native', layout: 'default', network_config_entity: '', discovery: { prefer_available: true, hide_unavailable_duplicates: true }, lights: [], scenes: [] }; }

  setConfig(config) {
    this._config = {
      title: 'Casambi',
      preset: config?.style || 'casambi-native',
      layout: 'default',
      auto_discover: false,
      discovery: { prefer_available: true, hide_unavailable_duplicates: true, exclude_sources: DEFAULT_EXCLUDE },
  ...
  render() {
    ...
    const preset = config.preset || config.style || 'casambi-native';
    ...
    const bodyClass = ['body', `layout-${layout}`].join(' ');
    ...
    this.innerHTML = `<ha-card class="cb preset-${preset} layout-${layout}">${this.css()}${header}${stats}<div class="${bodyClass}">${ordered}</div>${footer}</ha-card>`;
  ...
  renderScenes(scenes) {
    const hass = this._hass;
    return `<section class="section scenes-section"><h3><ha-icon icon="mdi:palette-outline"></ha-icon>Szenen</h3><div class="scenes">${scenes.length ? scenes.map((e) => `<button data-scene="${e}" title="${e}"><ha-icon icon="mdi:creation"></ha-icon>${stateName(hass.states[e]).replace(/^Casambi\s*/i, '')}</button>`).join('') : '<p>Keine Szenen gefunden. Bitte Discovery-Assistent im Editor nutzen.</p>'}</div></section>`;
  }
  ...
  renderLights(lights) {
    ...
    }).join('') : '<p>Keine Lichter gefunden. Bitte Discovery-Assistent im Editor nutzen.</p>'}</div></section>`;
  }
...
.preset-casambi-neon{--casambi-card-bg:radial-gradient(circle at top left,rgba(0,229,255,.25),transparent 28%),linear-gradient(145deg,#07131f,#102637);--casambi-accent:#00e5ff;--casambi-accent-2:#00ff99;--casambi-glow:0 0 34px rgba(0,229,255,.38)}
...
.preset-mushroom-dark{--casambi-card-bg:linear-gradient(145deg,#1b1f24,#111418);--casambi-panel:rgba(255,255,255,.07);--casambi-card-radius:18px;--casambi-glow:none}
.preset-bubble-compact{--casambi-card-radius:32px;--casambi-tile-radius:24px;--casambi-accent:#03a9f4;--casambi-accent-2:#00e5ff}
.preset-bubble-glass{--casambi-card-bg:linear-gradient(145deg,rgba(18,35,55,.72),rgba(12,18,30,.82));--casambi-panel:rgba(255,255,255,.12);--casambi-card-radius:32px;backdrop-filter:blur(12px);--casambi-glow:0 12px 42px rgba(0,0,0,.28)}
.preset-minimal{--casambi-card-bg:var(--ha-card-background,var(--card-background-color));--casambi-panel:transparent;--casambi-panel-2:rgba(127,127,127,.1);--casambi-glow:none;border-color:var(--divider-color)}
.head{display:flex;justify-content:space-between;gap:12px}.brand{display:flex;gap:12px;align-items:center}.logo{width:42px;height:42px;border-radius:14px;display:grid;place-items:center;background:linear-gradient(135deg,var(--casambi-accent),var(--casambi-accent-2));color:#03202d;font-weight:900}.title{font-weight:800;font-size:18px}.sub,footer,small,p{color:var(--casambi-muted);font-size:12px}.state{padding:8px 10px;border-radius:999px;background:var(--casambi-panel);height:max-content}.state i{display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:7px;background:#ef5350}.state.on i{background:var(--casambi-accent-2);box-shadow:0 0 10px var(--casambi-accent-2)}.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:16px 0}.stats b{background:var(--casambi-panel);border-radius:16px;padding:10px;text-align:center}.stats span{display:block;font-weight:400;color:var(--casambi-muted);font-size:11px}.section h3{font-size:14px;display:flex;gap:8px;align-items:center}.scenes{display:flex;gap:9px;flex-wrap:wrap}.scenes button{border:1px solid rgba(255,255,255,.14);border-radius:999px;padding:10px 13px;background:var(--casambi-panel);color:inherit;display:flex;gap:8px;cursor:pointer}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(215px,1fr));gap:10px}.tile{background:var(--casambi-panel);border-radius:var(--casambi-tile-radius);padding:12px}.tile.on{background:linear-gradient(145deg,rgba(21,200,255,.16),rgba(29,233,182,.1))}.tile.unav{opacity:.55}.tile-head{display:flex;justify-content:space-between;gap:8px}.tile strong{min-width:0;overflow:hidden;text-overflow:ellipsis}.tile button{border:0;border-radius:13px;background:var(--casambi-panel-2);color:inherit;width:36px;height:36px}.slider{display:grid;grid-template-columns:22px 1fr 54px;gap:8px;align-items:center;margin-top:10px}.slider input{width:100%;accent-color:var(--casambi-accent)}.slider em{text-align:right;font-style:normal;font-size:12px}.layout-wide .body{display:grid;grid-template-columns:1fr 1.4fr;gap:16px}.layout-compact .stats{grid-template-columns:repeat(2,1fr)}.layout-compact .grid{grid-template-columns:1fr}.layout-scenes-first .scenes-section{order:1}.layout-lights-first .lights-section{order:1}footer{display:flex;justify-content:space-between;gap:10px;margin-top:14px}@media(max-width:620px){.layout-wide .body{display:block}.stats{grid-template-columns:repeat(2,1fr)}}
</style>`; }
}

class CasambiBtCardEditor extends HTMLElement {
  setConfig(config) { this._config = { auto_discover: false, preset: 'casambi-native', layout: 'default', discovery: { prefer_available: true, hide_unavailable_duplicates: true, show_rejected_candidates: true, exclude_sources: DEFAULT_EXCLUDE }, ...(config || {}) }; this._rendered = false; this.render(); }
  ...
  applySelection(onlyAvailable = false) { const selector = onlyAvailable ? ':checked' : ':checked'; const lights = [...this.querySelectorAll(`input[data-kind="light"]${selector}`)].map((x) => x.value); const scenes = [...this.querySelectorAll(`input[data-kind="scene"]${selector}`)].map((x) => x.value); this._config = { ...this._config, lights, scenes, discovery_collapsed: true }; this.emit(); this.render(); }
  ...
  candHtml(type) {
    ...
    const selected = new Set(normList(this._config[type === 'light' ? 'lights' : 'scenes']));
  ...
  render() {
    ...
    const applySel = this.querySelector('#applySel'); if (applySel) applySel.onclick = () => this.applySelection(false);
...
window.customCards.push({ type: CARD_TAG, name: 'Casambi BT Card', description: 'Casambi Bluetooth lights and scenes with assisted discovery, presets and card_mod CSS hooks', preview: true });
